...
def choose_spot(tictactoe_game_board, player_symbol, player_type):
	"""
	...
	while turn_successful == False:
		...
		""" 
		this if-else statement is modified to make sure you're against a
		computer opponent. If you want to play another human, either remove 
		this if section and don't pass in player_type, or the code to always 
		pass in 'human' as the player_type.
		"""
		if player_type == 'human':
			spot_chosen = input("\nSelect your spot: \n")
		elif player_type == 'computer':
			spot_chosen = random.randint(1, 9)
...
def player_chooses_symbol(player1_symbol, player2_symbol):
	player_choice = input("X or O? ")
	if player_choice.lower() == player1_symbol:
		return 'human', 'computer'
	elif player_choice.lower() == player2_symbol:
...
# assign player types
player1_type, player2_type = player_chooses_symbol(player1, player2)


# this player_turn_elements dictionary is used in the second-option for-loop, 
# instead of assigning the symbol and type using an if statement
player_turn_elements = [
	{'symbol': player1, 'type': player1_type},
	{'symbol': player2, 'type': player2_type}
	]
...
while check_for_endstate(playing_board) == 'continue':
	...
	"""
	# You can also use this commented-out code, which loops through the two dictionary entries created above.
	for player_entry in player_turn_elements:
		player_symbol = player_entry['symbol']
		player_type = player_entry['type']
	"""
		...
		if check_for_endstate(playing_board) == 'continue':
			choose_spot(playing_board, player_symbol, player_type)
